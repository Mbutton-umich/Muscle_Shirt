...
//FOR THE LEDS
#define BITS_PER			24
#define RES_FRAME			250 // 800kHz > T= 1.25 us, datasheet lists 300 and also 250 us for frame reset length meaning 200 slots of PWM
#define MAX_LED 			50+10 //(Set this to ten over because the final LED glitches for some reason)
#define BRIGHTNESS 			0.25//Percentage of full brightness !0% gets up just to .200 A
#define BAR_SIZE 			10 //Number of LEDs that form a bar graph at a muscle site

//FOR THE ACTIVITY LOG
#define DEPTH				128 //Number of samples that show binary representation of activity changes
#define ACTIVITY_THRESHOLD	0.25 //Percentage of max EMG value (TODO: POWER of 2 will be faster)that is considered "Active" for the sake of recording activity
...
/* USER CODE BEGIN PV */
//FOR THE MOVING AVERAGE FILTER
uint8_t window_timeslot = 0;
uint8_t channel = 0;

//FOR THE ADC
//Stores raw ADC data for every channel, the length is 8 for the moving average
volatile uint32_t 	ADC_Val[EMG_COUNT][ADC_BUFFER_LENGTH];

//FOR THE LEDS
uint8_t bar_scalers[EMG_COUNT] = {3,4,4,5,3};	//Balances out the LED ouputs so they all look good on demo day (digital gain adjustments)
...
volatile uint8_t   PUMP[EMG_COUNT]; 						//Defines the current pump level 1-9, zero 0s nothing
...
uint8_t activity_log[EMG_COUNT][DEPTH]; 	//Array that updates columns whenever the pattern of muslce activation changes, depth is the column count, essnetially the memory for changes
uint8_t activity_index = 0;					//Location of the last updated activity column [0, DEPTH]
uint8_t push_ups = 0;
uint32_t muscle_sums[EMG_COUNT];

//FOR FLEX SENSOR
uint16_t strain_base =0;
...
//The DMA cannot be blocked mid stream or FUCKs into a deadlock
